...
from bitglitter.utilities.generalverifyfunctions import is_valid_directory, is_int_over_zero, proper_string_syntax, is_bool


def verify_read_parameters(file_to_input, output_path, encryption_key, scrypt_n, scrypt_r, scrypt_p, block_height_override,
    ...
    if os.path.splitext(file_to_input)[1] not in VALID_VIDEO_FORMATS and \
        ...
        raise ValueError(f'file_to_input argument {file_to_input} is not a valid format.  Only the following are allowed: '
